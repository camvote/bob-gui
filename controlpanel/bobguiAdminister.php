...
class bobguiAdminister extends frontControllerApplication
{
	...
	private function getVotersListFromLast ($ballot, &$lastBallotMetadata = false)
	{
		...
		$constraints = array (
			...
			'organisation' => $ballot['organisation'],
			'academicYear' => $this->academicYearFromStartDate (date ('Y-m-d')),
		);
		if (!$lastBallot = $this->databaseConnection->selectOne ($this->settings['database'], $this->settings['table'], $constraints, array (), false, 'instanceCompleteTimestamp DESC', 1)) {return false;}
	...
	private function understandText ($ballotUrl = false)
	{
		...
		<ol type=\"a\">
			...
			<li>(Alternatively, if you wish to conduct the count on your computer, take that count data, add the paper count data, and calculate the STV results. <a href=\"https://github.com/cusu/openstv/\" target=\"_blank\">OpenSTV</a> may be helpful for this.)</li>
			<li>The Returning Officer should then review the results and declare them when satisfied as to their accuracy.</li>
		</ol>
		";
